...
load_location_info <-
  function(auspostjson_source_file = 'https://www.matthewproctor.com/Content/postcodes/australian_postcodes.json') {
    location_info <- jsonify::from_json(auspostjson_source_file)
    colnames(location_info) <-
      janitor::make_clean_names(colnames(location_info))
    location_info <-
      location_info[!duplicated(location_info$postcode),]
