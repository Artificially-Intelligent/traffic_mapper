...
load_traffic_locations <-
  function(trafficlocation_source_uri = "https://vicroadsopendatastorehouse.vicroads.vic.gov.au/opendata/Traffic_Measurement/Bicycle_Volume_and_Speed/VicRoads_Bike_Site_Number_Listing.xlsx",
    ...
    download.file(trafficlocation_source_uri,
                  destfile = trafficlocation_source_files,
                  mode = "wb")
    ...
    bike_traffic_locations$lat <-
      as.numeric(str_extract(bike_traffic_locations$GPS, "[\\-\\+]\\d+.\\d+"))
    bike_traffic_locations$long <-
      as.numeric(str_extract(bike_traffic_locations$GPS, "\\ [\\-\\+]\\d+.\\d+"))
    
    colnames(bike_traffic_locations) <-
      janitor::make_clean_names(colnames(bike_traffic_locations))
    
    bike_traffic_locations_grouped <- bike_traffic_locations %>%
      mutate(
        location_type =  str_extract(str_extract(description, pattern = "\\(.+\\)")
                                     , pattern = "\\w+ \\w+"),
    ...
    colnames(bike_traffic_locations_grouped) <-
      janitor::make_clean_names(colnames(bike_traffic_locations_grouped))
...
load_traffic_locations_to_db <-
  function(trafficlocation_source_uri = "https://vicroadsopendatastorehouse.vicroads.vic.gov.au/opendata/Traffic_Measurement/Bicycle_Volume_and_Speed/VicRoads_Bike_Site_Number_Listing.xlsx",
           location_info = location_info) {
    traffic_locations <-
      load_traffic_locations(trafficlocation_source_uri = trafficlocation_source_uri,
                             location_info = location_info)
